...
const reducer1 = (acc, element) => {
  return element + acc
}
let totalPopulation = populations.reduce(reducer1, 0)
...
//Code Here
let myStrongest = monstersInYourPocket.filter(e => e['CP'] > 200)// = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
let orderTotals = orders.map(element => element.price + (element['price']*element['tax']) ) // Code here
...
let bobsTotal = purchases.reduce(reducer, 0)
